...
#my_species <- c('Infected_fish_busco', 'Infected_fish_busco', 'Infected_fish_busco', 'Infected_fish_busco', 'Ligula_busco_eu', 'Ligula_busco_eu', 'Ligula_busco_eu', 'Ligula_busco_eu', 'Control_fish_busco', 'Control_fish_busco', 'Control_fish_busco', 'Control_fish_busco')

my_species <- c(rep('Infected fish', 4),
                rep('Ligula', 4),
                rep('Control fish', 4))
...
my_percentage <- c(50.2, 37.9, 2.8, 9.1, 38.8, 53.3, 3.9, 4.0, 56.7, 32.6, 2.5, 8.2)
my_values <- c(1827, 1378, 103, 332, 99, 136, 10, 10, 2065, 1187, 91, 297)
...
figure <- ggplot() + 
  ...
  scale_fill_manual(values = my_colors,labels =c(" Complete (C) and single-copy (S)  ",
                                                 ...
                                                 " Fragmented (F)  ",
                                                 " Missing (M)")) +   
#  ggtitle(my_title) +  ## we don't really need ggtitle
  xlab("") + 
  ylab("\n%BUSCOs") + 
...
ggsave(figure, file=my_output, width = my_width, height = my_height, unit = my_unit, device = png)
print("Done")
